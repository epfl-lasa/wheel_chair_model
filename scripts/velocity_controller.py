...
class VelocityController():
    def __init__(self, attr_x=0, attr_y=0, dsController=False, freq=100, n_intSteps=20, dt_simu=0.1):
        ...
        name_topic_obs = "/Read_obstacle_Position/sphere"
        topic_list = rospy.get_published_topics()

        self.n_obs = 0
        for sublist in topic_list:
            for topic in sublist:
                if topic[:-1]==name_topic_obs:
                    self.n_obs += 1

        print("Number of Obstacles Detected: {}".format(self.n_obs))
        ...
        cmd_obstacles_pos = "/Read_obstacle_Position/Desired_Pose_sphere"
        self.pub_obs = [0]*self.n_obs
        for oo in range(self.n_obs):
            self.pub_obs[oo] = rospy.Publisher(cmd_obstacles_pos+str(oo+1), Pose2D, queue_size=10)
        ...
        is_circular_setup = True
        if is_circular_setup: # Circular initial set up
    ...
    def update_obsPosition(self, w_rand=0.004):
        for oo in range(self.n_obs):
            # TODO: dominance of first robot
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        print("Unexpected number of input arguments.")
    # else:
